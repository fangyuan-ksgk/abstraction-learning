...
class BaseHierDataset(ABC): 
    ...
    def _save(self): 
        ...
        with open(self.filepath, 'wb') as f:
            # Write header
            header = np.array([20241220, 2, len(self.sequences)], dtype=np.int32)
            f.write(header.tobytes())
            ...
            for seq, length in zip(self.sequences, self.lengths):
                ...
                f.write(np.int32(len(seq)).tobytes())
                for sub_seq in seq:
                    f.write(np.int32(len(sub_seq)).tobytes())
                    f.write(np.array(sub_seq, dtype=np.int32).tobytes())
    ...
    def _load_sequences_and_lengths(self) -> Tuple[List, List]:
        ...
        with open(self.filepath, 'rb') as f:
            ...
            if version == 1:
                ...
                for _ in range(n_sequences):
                    length = np.frombuffer(f.read(4), dtype=np.int32)[0]
                    num_sub_seqs = np.frombuffer(f.read(4), dtype=np.int32)[0]
                    
                    seq = []
                    for _ in range(num_sub_seqs):
                        sub_seq_len = np.frombuffer(f.read(4), dtype=np.int32)[0]
                        sub_seq = np.frombuffer(f.read(sub_seq_len * 4), dtype=np.int32).tolist()
                        seq.append(sub_seq)

                    lengths.append(int(length))
                    sequences.append(seq)
            else:
                raise ValueError(f"Unknown data format version: {version}")
    ...
    def update_abstract_params(self, L: Optional[int] = None, K: Optional[int] = None): 
        self.L = L if L is not None else self.L
        self.K = K if K is not None else self.K
        self.lengths = [compute_hier_seq_len(seq[0], self.L, self.K) for seq in self.sequences]
